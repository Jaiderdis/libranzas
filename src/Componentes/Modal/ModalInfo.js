...
const ModalInfo = ({ data, show, close }) => {
    ...
    return (
        <>
            ...
            <div className={`fixed z-50 overflow-y-auto top-0 w-full left-0 ${(!show ? 'hidden' : '')}`} onClick={close} >
                <div className="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div onClick={stopPropagation} className="inline-block align-center rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                        <div className="bg-primary-500 text-white px-4 py-3 flex justify-between">
                            <h2>{data.Title}</h2>
                            <div><span className="cursor-pointer" onClick={close}><FaXmark /></span></div>
                        </div>
                        ...
                        <div className="bg-gray-200 px-4 py-3 text-right">
                            <button type="button" className="py-2 px-4 bg-gray-500 text-white rounded hover:bg-gray-700 mr-2" onClick={close}><i className="fas fa-times"></i> Cerrar</button>
